...
func TestHealthAndReadyEndpoints(t *testing.T) {
	handler := NewHandler(fakeProvider{}, fakeAuthService{statusOut: auth.StatusOutput{Exists: true, Expired: false}}, "secret", logging.NewRecorder(), time.Now)
	...
	notReady := NewHandler(fakeProvider{}, fakeAuthService{statusOut: auth.StatusOutput{Exists: false}}, "secret", logging.NewRecorder(), time.Now)
	rec := httptest.NewRecorder()
...
func TestProviderAndAuthErrors(t *testing.T) {
	handler := NewHandler(fakeProvider{modelsErr: core.NewError("MODELS_FAILED", http.StatusBadGateway, "models failed", nil), chatErr: core.NewError("CHAT_FAILED", http.StatusBadGateway, "chat failed", nil), streamErr: core.NewError("STREAM_FAILED", http.StatusBadGateway, "stream failed", nil)}, fakeAuthService{statusErr: core.NewError("STATUS_FAILED", http.StatusInternalServerError, "status failed", nil)}, "secret", logging.NewRecorder(), time.Now)
	rec := httptest.NewRecorder()
