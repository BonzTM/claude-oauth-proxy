# [1.1.5] Release Notes - 2026-04-01

## Release Summary

Fingerprint maintenance release. Updates the Claude Code client version defaults from `2.1.87` to `2.1.89` so that proxy requests match the latest Claude Code CLI release. No functional changes, new features, or configuration schema changes are included.

## Changed

- **Claude Code version bump** — `DefaultCCVersion` updated from `2.1.87` to `2.1.89` in `internal/runtime/config.go`.
- **User-Agent default** — `DefaultUserAgent` updated from `claude-cli/2.1.87 (external, cli)` to `claude-cli/2.1.89 (external, cli)`.
...
- **Extraction script** — Comment example in `scripts/extract-cc-fingerprint.sh` updated to `2.1.89`.
...
helm repo update
helm upgrade --install claude-oauth-proxy claude-oauth-proxy/claude-oauth-proxy --version 1.1.5
```

Alternatively, set the new version via environment variables on an existing binary without rebuilding:

```bash
export CLAUDE_OAUTH_PROXY_CC_VERSION=2.1.89
export CLAUDE_OAUTH_PROXY_CC_USER_AGENT="claude-cli/2.1.89 (external, cli)"
```
...
- Other known issues from 1.1.4 remain unchanged; this patch does not add new user-facing behavior.

## Compatibility and Migration

- No configuration changes or migration steps are required for this release.
- Existing 1.1.4 deployments can upgrade directly to 1.1.5.
...
- Compare changes: https://github.com/BonzTM/claude-oauth-proxy/compare/1.1.4...1.1.5
- Full changelog: https://github.com/BonzTM/claude-oauth-proxy/commits/1.1.5
