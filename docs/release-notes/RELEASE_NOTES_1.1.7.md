# [1.1.7] Release Notes - 2026-04-03

## Release Summary

Fingerprint maintenance release. Updates the Claude Code client version defaults from `2.1.90` to `2.1.91`.

## Changed

- **Claude Code version bump** — `DefaultCCVersion` updated from `2.1.90` to `2.1.91` in `internal/runtime/config.go`.
- **User-Agent default** — `DefaultUserAgent` updated from `claude-cli/2.1.90 (external, cli)` to `claude-cli/2.1.91 (external, cli)`.
...
- **Extraction script** — Comment example in `scripts/extract-cc-fingerprint.sh` updated to `2.1.91`.
...
helm repo update
helm upgrade --install claude-oauth-proxy claude-oauth-proxy/claude-oauth-proxy --version 1.1.7
```

Alternatively, set the new version via environment variables on an existing binary without rebuilding:

```bash
export CLAUDE_OAUTH_PROXY_CC_VERSION=2.1.91
export CLAUDE_OAUTH_PROXY_CC_USER_AGENT="claude-cli/2.1.91 (external, cli)"
```
...
- Other known issues from 1.1.6 remain unchanged; this patch does not add new user-facing behavior.

## Compatibility and Migration

- No configuration changes or migration steps are required for this release.
- Existing 1.1.6 deployments can upgrade directly to 1.1.7.
...
- Compare changes: https://github.com/BonzTM/claude-oauth-proxy/compare/1.1.6...1.1.7
- Full changelog: https://github.com/BonzTM/claude-oauth-proxy/commits/1.1.7
